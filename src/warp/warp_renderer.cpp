...
"                    dSrc = max(dSrc, gDepth.SampleLevel(gPt, suv - md, 0).r);\n"
"                }\n"
"                // Path-aware reject: a fixed dilation can't keep up with a fast turn -- the gather\n"
"                // travels far and the gun can sit ANYWHERE along the path, not just at the landing\n"
"                // point. Sample a few points between this world pixel and its warped source; if the\n"
"                // gun lies on that line the source is occluded by it. The path length is the warp\n"
"                // magnitude, so this self-scales: tight when slow, wide when fast.\n"
"                float2 pstep = i.uv - suv;\n"
"                dSrc = max(dSrc, gDepth.SampleLevel(gPt, suv + pstep * 0.25f, 0).r);\n"
...
"        float dHere = gDepth.SampleLevel(gPt, i.uv, 0).r;\n"
"        // Same dilated weapon-lock as modes 4/5: a single-sample near test lets the gun's soft render-res\n"
"        // silhouette read as world (-> it gets reprojected -> outline ghost), and a high near-cut leaves\n"
"        // the whole gun un-locked (-> translation-reprojected -> doubling). Dilate the mask + fill the\n"
"        // optic hole so the gun (and its lens) lock as one unit.\n"
"        float dMax = dHere;\n"
"        if (gMaskDilate > 0.0f) {\n"
"            float2 md = float2(gMaskDilate / gAspect, gMaskDilate);\n"
"            dMax = max(dMax, gDepth.SampleLevel(gPt, i.uv + float2( md.x,0),0).r);\n"
...
"        bool nearHere = dMax > gNearCut;\n"
"        if (!nearHere && gWeaponDilate > 0.0f) {\n"
"            float2 r = float2(gWeaponDilate / gAspect, gWeaponDilate); float2 dg = r * 0.70710678f; int enc = 0;\n"
"            if (gDepth.SampleLevel(gPt, i.uv+float2( r.x,0),0).r>gNearCut && gDepth.SampleLevel(gPt, i.uv+float2(-r.x,0),0).r>gNearCut) enc++;\n"
"            if (gDepth.SampleLevel(gPt, i.uv+float2(0, r.y),0).r>gNearCut && gDepth.SampleLevel(gPt, i.uv+float2(0,-r.y),0).r>gNearCut) enc++;\n"
"            if (gDepth.SampleLevel(gPt, i.uv+float2( dg.x,dg.y),0).r>gNearCut && gDepth.SampleLevel(gPt, i.uv+float2(-dg.x,-dg.y),0).r>gNearCut) enc++;\n"
"            if (gDepth.SampleLevel(gPt, i.uv+float2( dg.x,-dg.y),0).r>gNearCut && gDepth.SampleLevel(gPt, i.uv+float2(-dg.x,dg.y),0).r>gNearCut) enc++;\n"
...
"        }\n"
"        if (nearHere) {\n"
"            suv = i.uv;                                   // near-field weapon (+ its lens): screen-lock\n"
"        } else {\n"
"            float Zown = 1.0f / max(dHere, 1e-4f);\n"
"            uint steps = max(gRaySteps, 1u);\n"
"            float tMax = Zown * 1.5f;                       // march a bit past this pixel's own depth\n"
"            float dt = tMax / steps;\n"
"            float t = 0.0f, tPrev = 0.0f; bool hit = false; float2 puv = i.uv;\n"
"            [loop] for (uint k=0; k<steps; k++) {\n"
"                t += dt;\n"
"                float3 P = ro + f * t;\n"
"                if (P.z <= 1e-4f) { tPrev = t; continue; }\n"
"                float2 q = float2((P.x/P.z/tw + 1.0f)*0.5f, (1.0f - P.y/P.z/th)*0.5f);\n"
"                float sd = gDepth.SampleLevel(gPt, q, 0).r;\n"
"                if (sd > gNearCut) { tPrev = t; continue; }   // ray passing through the gun: skip it and\n"
"                                                              // keep marching to the world behind (no ghost)\n"
"                float diff = P.z - 1.0f/max(sd,1e-4f);        // marched past the surface => hit\n"
"                if (diff > 0.0f) {\n"
"                    float a = tPrev, b = t;                    // binary-refine the crossing to kill the\n"
"                    [loop] for (int r=0; r<6; r++) {           // stair-step laddering of a coarse march\n"
"                        float m = 0.5f*(a+b); float3 Pm = ro + f*m;\n"
"                        float2 qm = float2((Pm.x/Pm.z/tw+1.0f)*0.5f, (1.0f - Pm.y/Pm.z/th)*0.5f);\n"
"                        float sdm = gDepth.SampleLevel(gPt, qm, 0).r;\n"
"                        if (Pm.z - 1.0f/max(sdm,1e-4f) > 0.0f) b = m; else a = m;\n"
"                    }\n"
"                    float3 Pf = ro + f*b;\n"
"                    puv = float2((Pf.x/Pf.z/tw+1.0f)*0.5f, (1.0f - Pf.y/Pf.z/th)*0.5f);\n"
"                    hit = true; break;\n"
"                }\n"
"                tPrev = t;\n"
"            }\n"
